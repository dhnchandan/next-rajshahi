...
export function Testimonials({
  ...
  description = "Trusted by businesses and startups to deliver exceptional results",
  testimonials,
  className,
}: TestimonialsProps) {
  return (
          ...
          {testimonials.map((testimonial, index) => (
            <motion.div key={index} variants={item}>
              <Card className="group relative h-full overflow-hidden border-border bg-card p-4 transition-all duration-300 sm:p-6 hover:border-primary/50">
                <Quote className="absolute top-3 right-3 h-8 w-8 text-muted/10 sm:top-4 sm:right-4 sm:h-12 sm:w-12 group-hover:text-primary/10" />
