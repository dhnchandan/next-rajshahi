...
import type { MetricItem } from "@/components/parts/types";
...
export function Metrics({
  ...
  description,
  metrics,
  ctaLabel = "concierge insight desk",
  ctaDescription = "Curated weekly digests keep leaders aligned without dashboards.",
  ctaButtonText = "Request a sample",
  className,
}: MetricsProps) {
  return (
      ...
      <div className="mx-auto max-w-6xl space-y-12">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full border border-border/40 bg-background/70 text-foreground/80">
              <Users className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm uppercase tracking-[0.25em] text-foreground/60">{ctaLabel}</p>
              <p className="text-base text-foreground/80">{ctaDescription}</p>
            </div>
          </div>
          <Button
            size="lg"
            variant="ghost"
            className="h-11 rounded-full border border-border/40 bg-background/70 px-6 text-sm uppercase tracking-[0.2em] text-foreground/70 backdrop-blur"
          >
            {ctaButtonText}
          </Button>
        </motion.div>
